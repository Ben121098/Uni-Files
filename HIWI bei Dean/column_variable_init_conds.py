...
def template_model(init_ind=0):
    ...
    e0_Param_N2max_sharp = trajectory['Flowsheet.e0_Param_N2max_sharp'][init_ind]#*0+1
    e0_Param_PCsig_sharp = trajectory['Flowsheet.e0_Param_PCsig_sharp'][init_ind]#*0+1
    e0_Param_SVmax_sharp = trajectory['Flowsheet.e0_Param_SVmax_sharp'][init_ind]#*0+1
    e0_Param_Vmax_sharp = trajectory['Flowsheet.e0_Param_Vmax_sharp'][init_ind]#*0+1
    e0_Param_Vmin_sharp = trajectory['Flowsheet.e0_Param_Vmin_sharp'][init_ind]#*0+1
    e0_Param_mid_sharp = trajectory['Flowsheet.e0_Param_mid_sharp'][init_ind]#*0+1
...
def template_simulator(model,init_ind=0,use_new_z=False,z_new=-1):
    """
    Here could be the doc
    """
    ...
    elif use_new_z and (max(z_new.shape) == 467):
        relevant_keys = list(simulator.z0.keys())[1:] # first key of the dict is irrelevant
        for ind, zkey in enumerate(relevant_keys):
            simulator.z0[zkey] = z_new[ind]
...
if __name__ == "__main__":
    ...
    # index = 0
    # z_new_dummy = np.zeros((467))
    # model = template_model(init_ind=index)
    # simulator = template_simulator(model,init_ind=index,use_new_z=False,z_new=z_new_dummy)
    ...
    # uu = ca.SX.sym("uu",145)
    # zz = ca.SX.sym("zz",467)
    # rf = ca.Function("rf",[zz,xx,uu],[simulator.model._alg_fun(xx,uu,zz,0,0,0)],
    #                  ["zz","xx","uu"],["rf"])
    # G = ca.rootfinder("G","newton",rf)
    
    # computation_finished = False
    # versuche = 1
    # x_old = pickle.load(open("Data/x0.dat","rb"))
    # u_old = pickle.load(open("Data/u0.dat","rb"))
    # z_old = pickle.load(open("Data/z0.dat","rb"))
    
    
    # while not computation_finished:
    #     try:
    #         for idx in range(10):
    #             # ut = u_from_trajectory(idx=1000)
    #             u0 = simulator.u0.master
    #             update_u(simulator,trajectory=-1,idx=index+idx,getoutput=False)
    #             # u0 = simulator.u0
    ...
    #         print("\n",idx,"!!!!\n")
    #         versuche += 1
    #         print("Versuch Nr. {}".format(versuche))
    #         if versuche >3:
    #             computation_finished = True
    #         z0 = z_old
    #         x0 = x_old
    #         u0 = u_old
    #         z_rf = G(z0,x0,u0).full()
    #         z_rf = z_rf.reshape(467,)
    #         simulator = template_simulator(model,init_ind=index,use_new_z=True
    #                                        ,z_new=z_rf)
    #         simulator.set_param(**params_simulator)
    #         simulator.setup()
    #         z_old = z_rf
